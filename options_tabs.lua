...
Tabs.GROUP_TREE = {
    ...
    { title = "Group", groups = {
        ...
        { "RAID_WARNING", "Raid warning" }, { "BATTLEGROUND", "Battleground" },
        { "BATTLEGROUND_LEADER", "Battleground leader" },
    } },
    { title = "Creatures", groups = {
    ...
    { title = "Combat and progress", groups = {
        { "COMBAT_XP_GAIN", "Experience" }, { "COMBAT_HONOR_GAIN", "Honour" },
        { "COMBAT_FACTION_CHANGE", "Reputation" }, { "COMBAT_MISC_INFO", "Other combat" },
        { "SKILL", "Skill ups" }, { "LOOT", "Loot" }, { "MONEY", "Money" },
...
function Tabs.Build(flow)
    ...
    sec:Hint("One page per chat tab. Names, colours and routing are part of your shared chat "
          .. "configuration - set them once on any character and every character gets them.")
    statusHint(sec, Tabs.Status)

    -- ── The sub-tab strip: one button per chat tab, plus + Add Tab ───────
    local navRow = sec:AddRow({ vAlign = "center" })
    Tabs._nav = {}
    for i = 1, Tabs.MAX_PAGES do
        Tabs._nav[i] = navRow:Button({
            text = "", width = 110, variant = "quiet",
            onClick = function()
    ...
    sec:Hint("A new tab is created in your shared configuration and appears on every character "
          .. "at once. Removing asks twice: the first press arms the button.")

    -- ── The selected page ───────────────────────────────────────────────
    sec:AddSeparator()
    local nameRow = sec:AddRow({ vAlign = "center" })
    nameRow:Label("Tab name")
    bind("tabs.name")
    reg(nameRow:EditBox({
        width = 180,
        get = function()
    ...
    sec:Hint("\"Automatic\" gives a tab the colour of the channel it is for, and the accent "
          .. "colour when it carries more than one. Channel and message colours themselves "
          .. "live in General - they belong to the channel, not to the tab.")
    ...
    sec:AddRow({ vAlign = "center" }):Button({
    ...
    for _, spec in ipairs(Tabs.TOGGLES) do
        bind(spec.binding)
        Tabs._toggles[spec.key] = reg(sec:Checkbox({
    ...
    -- ── Message groups (the checkbox tree) ──────────────────────────────
    sec:AddSeparator()
    sec:Hint("WHICH MESSAGES COME HERE:")
    bind("tabs.group")
    Tabs._groupBoxes = {}
    for _, branch in ipairs(Tabs.GROUP_TREE) do
        sec:Hint(branch.title)
        for _, g in ipairs(branch.groups) do
            local group, label = g[1], g[2]
            Tabs._groupBoxes[group] = reg(sec:Checkbox({
                label = label,
                get = function()
                    local C, id = ns.Config, Tabs.Selected()
    ...
    -- ── Channel routing ─────────────────────────────────────────────────
    sec:AddSeparator()
    sec:Hint("WHICH CHANNELS COME HERE:")
    bind("tabs.channel")
    Tabs._chanBoxes = {}
    for i = 1, MAX_CHANNEL_ROUTES do
        Tabs._chanBoxes[i] = reg(sec:Checkbox({
            label = "",
            get = function()
                local C, id = ns.Config, Tabs.Selected()
    ...
    reg(sec:Checkbox({
        label = "Route addon messages here",
        tooltip = "Off leaves every line exactly where it would have been without the addon "
               .. "tab - nothing is classified at all.",
...
function Tabs.Refresh()
    ...
    for i = 1, Tabs.MAX_PAGES do
        ...
        local id = ids[i]
        if btn then
            local text = id and Tabs.PageLabel(id) or ""
            if id == sel and text ~= "" then text = "[" .. text .. "]" end
            if type(btn.SetText) == "function" then pcall(btn.SetText, btn, text)
            elseif btn._label and type(btn._label.SetText) == "function" then
                pcall(btn._label.SetText, btn._label, text)
            end
            if btn._opts then btn._opts.text = text end
            if type(btn.Show) == "function" and type(btn.Hide) == "function" then
                pcall(id and btn.Show or btn.Hide, btn)
            end
        end
    end
    local rm = Tabs._removeBtn
    if rm then
        local text = sel and Tabs.RemoveLabel(sel) or "Remove this tab"
        if type(rm.SetText) == "function" then pcall(rm.SetText, rm, text) end
        if rm._label and type(rm._label.SetText) == "function" then
            pcall(rm._label.SetText, rm._label, text)
        end
        if rm._opts then rm._opts.text = text end
    end
    ...
    for i = 1, MAX_CHANNEL_ROUTES do
        ...
        local row = rows[i]
        if box then
            local text = row and Options.ChannelRowLabel(row) or ""
            if box._opts then box._opts.label = text end
            if box._label and type(box._label.SetText) == "function" then
                pcall(box._label.SetText, box._label, text)
            elseif type(box.SetText) == "function" then pcall(box.SetText, box, text) end
